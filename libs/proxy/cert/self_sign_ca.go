...
func createCert() (*rsa.PrivateKey, *x509.Certificate, error) {
	...
	template := &x509.Certificate{
		...
		Subject: pkix.Name{
			CommonName:   "Gateweb CA",
			Organization: []string{"Gateweb"},
		},
...
func getStorePath(path string) (string, error) {
	if path == "" {
		...
		path = filepath.Join(homeDir, ".gateweb")
	}
...
func (ca *SelfSignCA) caFile() string {
	return filepath.Join(ca.StorePath, "gateweb-ca.pem")
}

// The certificate in PEM format.
func (ca *SelfSignCA) caCertFile() string {
	return filepath.Join(ca.StorePath, "gateweb-ca-cert.pem")
}

func (ca *SelfSignCA) caCertCerFile() string {
	return filepath.Join(ca.StorePath, "gateweb-ca-cert.cer")
}
...
// DummyCert creates a leaf certificate signed by the root CA for the given host.
