...
import (
	...
	"gateweb/libs/proxy/internal/helper"
	"io"
	"net"
	"net/http"
	"strings"
...
func (a *attacker) initHttpsDialFn(req *http.Request) {
	...
	connCtx.dialFn = func(ctx context.Context) error {
		_, err := a.httpsDial(ctx, req, connCtx)
		if err != nil {
			return err
		}
...
func (a *attacker) httpsDial(ctx context.Context, req *http.Request, connCtx *ConnContext) (net.Conn, error) {
	proxy := a.proxy
...
func (a *attacker) httpsLazyAttack(ctx context.Context, cconn net.Conn, req *http.Request, connCtx *ConnContext) {
	log := log.WithFields(log.Fields{
